...
export function SettingsPage({ onUnsavedChangesChange, onResetRequest }: SettingsPageProps) {
  ...
  const [pendingConcurrent, setPendingConcurrent] = useState<3 | null>(null);
  ...
  return (<div className="space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Tooltip>
              ...
              <TooltipContent side="top">
                <p className="text-xs">How many tracks to download simultaneously</p>
              </TooltipContent>
            </Tooltip>
          </div>
          <Select
            ...
            onValueChange={(value) => {
              const numValue = parseInt(value) as 1 | 2 | 3;
              if (numValue === 3) {
                // Show warning for option 3
                setShowConcurrentWarning(true);
                setPendingConcurrent(3);
              } else {
    ...
    <Dialog open={showConcurrentWarning} onOpenChange={setShowConcurrentWarning}>
      <DialogContent className="max-w-md [&>button]:hidden">
        <DialogHeader>
          <DialogTitle>High Concurrency Warning</DialogTitle>
          <DialogDescription className="pt-2">
            Downloading 3 tracks simultaneously may trigger rate limits on some services, causing downloads to fail. Use this setting only if you have a stable connection and are willing to retry failed downloads.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="outline" onClick={() => setShowConcurrentWarning(false)}>Cancel</Button>
          <Button onClick={() => {
            if (pendingConcurrent) {
              setTempSettings(prev => ({ ...prev, concurrentDownloads: pendingConcurrent }));
            }
            setShowConcurrentWarning(false);
            setPendingConcurrent(null);
          }}>I Understand</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
